...
class Mysqltunnel  {
	...
	protected $allowTestMenu = true;
	protected $use_mysqli = null;
	...
	public function __construct(){
		...
		error_reporting(0);
		set_time_limit(0);
		if ($this->phpversion_int() < 50300){
			set_magic_quotes_runtime(0);
		}

		if ($this->phpversion_int() < 40005) {
			$this->EchoHeader(201);
			echo $this->GetBlock("unsupported php version");
			exit();
		}

		if ($this->phpversion_int() < 40010) {
			global $HTTP_POST_VARS;
			$_POST = &$HTTP_POST_VARS;	
		}

		if (!isset($_POST["actn"]) || !isset($_POST["host"]) || !isset($_POST["port"]) || !isset($_POST["login"])) {
			$testMenu = $this->allowTestMenu;
			if (!$testMenu){
				$this->EchoHeader(202);
				echo $this->GetBlock("invalid parameters");
				exit();
			}
		}
		...
		if(!$testMenu) {
			...
			if (!function_exists("mysql_connect") && !function_exists("mysqli_connect")) {
				$this->EchoHeader(203);
				echo $this->GetBlock("MySQL not supported on the server");
				exit();
			}
			$errno_c = 0;
			$hs = $_POST["host"];
			if ($this->use_mysqli) {
				...
				if ($this->phpversion_int() >= 50005){  // for unicode database name
					mysqli_set_charset($conn, 'UTF8');
				}

				if($errno_c > 0) {
					$this->EchoHeader($errno_c);
					echo $this->GetBlock(mysqli_connect_error($conn));
					exit;
				}
				...
				$this->EchoHeader($errno_c);
				if($errno_c > 0) {
					echo $this->GetBlock(mysqli_error($conn));
				} elseif($_POST["actn"] == "C") {
					$this->EchoConnInfo($conn);
				} elseif($_POST["actn"] == "Q") {
					for($i=0;$i<count($_POST["q"]);$i++) {
						$query = $_POST["q"][$i];
						if($query == "") continue;
						if ($this->phpversion_int() < 50400){  
						...
						$this->EchoResultSetHeader($errno, $affectedrows, $insertid, $numfields, $numrows);
						if($errno > 0)
							echo $this->GetBlock(mysqli_error($conn));
						else {
							if($numfields > 0) {
								$this->EchoFieldsHeader($res, $numfields);
								$this->EchoData($res, $numfields, $numrows);
							} else {
								if($this->phpversion_int() >= 40300)
									echo $this->GetBlock(mysqli_info($conn));
								else
									echo $this->GetBlock("");
							}
						}
				...
				$errno_c = mysql_errno();
				if ($this->phpversion_int() >= 50203){  // for unicode database name
				...
				$this->EchoHeader($errno_c);
				if($errno_c > 0) {
					echo $this->GetBlock(mysql_error());
				} elseif($_POST["actn"] == "C") {
					$this->EchoConnInfo($conn);
				} elseif($_POST["actn"] == "Q") {
					for($i=0;$i<count($_POST["q"]);$i++) {
						...
						if($errno > 0)
							...
							if($numfields > 0) {
								...
								if($this->phpversion_int() >= 40300)
									echo $this->GetBlock(mysql_info($conn));
								else
									echo $this->GetBlock("");
							}
						}
...
<?php 
	...
	public function phpversion_int() {
		list($maVer, $miVer, $edVer) = preg_split("(/|\.|-)", phpversion());
		return $maVer*10000 + $miVer*100 + $edVer;
	}

	public function GetLongBinary($num) {
		return pack("N",$num);
	}

	public function GetShortBinary($num){
		return pack("n",$num);
	}

	public function GetDummy($count) {
		$str = "";
		for($i=0;$i<$count;$i++)
			$str .= "\x00";
		return $str;
	...
	public function EchoHeader($errno)	{
		$str = $this->GetLongBinary(1111);
		$str .= $this->GetShortBinary(202);
		$str .= $this->GetLongBinary($errno);
		$str .= $this->GetDummy(6);
		echo $str;
	}

	public function EchoConnInfo($conn)	{
		if ($GLOBALS['use_mysqli']) {
			$str = $this->GetBlock(mysqli_get_host_info($conn));
			$str .= $this->GetBlock(mysqli_get_proto_info($conn));
			$str .= $this->GetBlock(mysqli_get_server_info($conn));
			echo $str;
		} else {
			$str = $this->GetBlock(mysql_get_host_info($conn));
			$str .= $this->GetBlock(mysql_get_proto_info($conn));
			$str .= $this->GetBlock(mysql_get_server_info($conn));
			echo $str;
		}
	}

	public function EchoResultSetHeader($errno, $affectrows, $insertid, $numfields, $numrows)	{
		$str = $this->GetLongBinary($errno);
		$str .= $this->GetLongBinary($affectrows);
		$str .= $this->GetLongBinary($insertid);
		$str .= $this->GetLongBinary($numfields);
		$str .= $this->GetLongBinary($numrows);
		$str .= $this->GetDummy(12);
		echo $str;
	}

	public function EchoFieldsHeader($res, $numfields)	{
		$str = "";
		for( $i = 0; $i < $numfields; $i++ ) {
			if ($GLOBALS['use_mysqli']) {
				$finfo = mysqli_fetch_field_direct($res, $i);
				$str .= $this->GetBlock($finfo->name);
				$str .= $this->GetBlock($finfo->table);

				$type = $finfo->type;
				$length = $finfo->length;

				$str .= $this->GetLongBinary($type);

				$intflag = $finfo->flags;
				$str .= $this->GetLongBinary($intflag);

				$str .= $this->GetLongBinary($length);
			} else {
				$str .= $this->GetBlock(mysql_field_name($res, $i));
				$str .= $this->GetBlock(mysql_field_table($res, $i));
	...
	public function EchoData($res, $numfields, $numrows)	{
		for( $i = 0; $i < $numrows; $i++ ) {
			$str = "";
			$row = null;
			if ($GLOBALS['use_mysqli'])
	...
	public function doSystemTest()	{
		...
		output("PHP version >= 4.0.5", $this->phpversion_int() >= 40005, array("Yes", "No"));
		...
		if ($this->phpversion_int() >= 40302 && substr($_SERVER["SERVER_SOFTWARE"], 0, 6) == "Apache" && function_exists("apache_get_modules")){
